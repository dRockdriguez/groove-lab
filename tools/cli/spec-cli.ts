...
program
  ...
  .action((spec) => runPrompt("tools/prompts/analyze-spec.md", spec))

program
  .command("test <spec>")
  .description("Generate tests from spec")
  .action((spec) => runPrompt("tools/prompts/generate-tests.md", spec))

program
  .command("implement <spec>")
  .description("Implement feature from spec")
  .action((spec) => runPrompt("tools/prompts/implement-feature.md", spec))

program
  .command("verify <spec>")
  .description("Verify implementation against spec")
  .action((spec) => runPrompt("tools/prompts/verify-spec.md", spec))

program
  .command("run <spec>")
  .description("Run full workflow")
  .action((spec) => {
    runPrompt("tools/prompts/analyze-spec.md", spec)
    runPrompt("tools/prompts/generate-tests.md", spec)
    runPrompt("tools/prompts/implement-feature.md", spec)
    runPrompt("tools/prompts/verify-spec.md", spec)
  })
