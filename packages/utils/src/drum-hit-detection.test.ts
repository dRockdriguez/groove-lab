...
import { buildHitLookup, validateDrumHit } from './index';
...
describe('Drum Hit Detection', () => {
  ...
  describe('validateDrumHit', () => {
    ...
    describe('HIT classification', () => {
      it('classifies a hit within tolerance window as "hit"', () => {
        const result = validateDrumHit(36, 25, lookup, 150);
      ...
      it('classifies a hit at tolerance boundary as "hit"', () => {
    ...
    describe('tolerance window', () => {
      ...
      it('uses default 150ms tolerance when not specified', () => {
        const result = validateDrumHit(36, 140, lookup); // 140ms from 0, within default
        expect(result?.classification).toBe('hit');
      });
    });
